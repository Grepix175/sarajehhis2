...
class Hospital_code_entry_model extends CI_Model
{
	var $table = 'hms_hospital_code_entry';
	var $column = array('hms_hospital_code_entry.id', 'hms_hospital_code_entry.hos_code_id', 'hms_hospital_code_entry.item_desc_id', 'hms_hospital_code_entry.unit_id', 'hms_hospital_code_entry.manuf_company ', 'hms_hospital_code_entry.status', 'hms_hospital_code_entry.created_date', 'hms_hospital_code_entry.modified_date');
	var $order = array('hms_hospital_code_entry.id' => 'desc');
	...
	private function _get_datatables_query()
	{
		...
		// $this->db->select("hms_hospital_code_entry.*,hms_medicine_racks.rack_no,hms_medicine_racks.id as rack_id,hms_medicine_company.company_name");
	...
	{
		// $this->db->select("hms_hospital_code_entry.*,hms_medicine_company.company_name, hms_medicine_unit.medicine_unit,hms_medicine_racks.rack_no as rack_nu,hms_medicine_type.medicine_type_name");
		// $this->db->from('hms_hospital_code_entry');

		// $this->db->where('hms_hospital_code_entry.id', $id);
		// $this->db->where('hms_hospital_code_entry.is_deleted', '0');
		// $this->db->join('hms_medicine_company', 'hms_medicine_company.id=hms_hospital_code_entry.manuf_company', 'left');

		// $this->db->join('hms_medicine_type', 'hms_medicine_type.id=hms_hospital_code_entry.medicine_type', 'left');
		// $this->db->join('hms_medicine_racks', 'hms_medicine_racks.id=hms_hospital_code_entry.rack_no', 'left');
		// $this->db->join('hms_medicine_unit', 'hms_medicine_unit.id=hms_hospital_code_entry.unit_id', 'left');
		$this->db->select("hms_hospital_code_entry.*, hms_medicine_company.company_name, hms_medicine_unit.medicine_unit, hms_item_desc.item_desc, hms_hospital_code.hospital_code");
		$this->db->from($this->table);
	...
	public function save()
	{
		...
		$data = array(
			...
			"hos_code_id" => $post['hos_code_id'],
			"unit_id" => $post['unit_id'],
			"item_desc_id" => $post['item_desc_id'],
			"manuf_company" => $post['manuf_company'],
			"qty" => $post['qty'],
			"hospital_rate" => $post['hospital_rate'],
			"status" => $post['status'],
		);
		if (!empty($post['data_id']) && $post['data_id'] > 0) {
			...
			$this->db->update('hms_hospital_code_entry', $data);
			...
			$this->db->insert('hms_hospital_code_entry', $data);
