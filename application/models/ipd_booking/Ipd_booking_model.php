    ...
    public function save()
    {
        ...
        if (!empty($post['data_id']) && $post['data_id'] > 0) {
            ...
            if (isset($post['id_number'])) {
                $panel_id_no = $post['id_number'];
            } else {
                $panel_id_no = '';
            }
            if (isset($post['company_name'])) {
                $panel_name = $post['company_name'];
            } else {
                $panel_name = '';
            }
            ...
            $data = array(
                ...
                'branch_id' => $user_data['parent_id'],
                'attend_doctor_id' => $post['attended_doctor'], // Duplicate; kept one
                'mlc' => $post['mlc'],
                ...
                // New fields added from the first array
                // 'simulation_id' => isset($post['simulation_id']) ? $post['simulation_id'] : '',
                // 'relation_type' => isset($post['relation_type']) ? $post['relation_type'] : '',
                // 'relation_name' => isset($post['relation_name']) ? $post['relation_name'] : '',
                // 'relation_simulation_id' => isset($post['relation_simulation_id']) ? $post['relation_simulation_id'] : '',
                // 'mobile_no' => isset($post['mobile']) ? $post['mobile'] : '',
                // 'age_m' => isset($post['age_m']) ? $post['age_m'] : 0,
                // 'age_d' => isset($post['age_d']) ? $post['age_d'] : 0,
                // 'gender' => isset($post['gender']) ? $post['gender'] : '',
                // 'age_y' => isset($post['age_y']) ? $post['age_y'] : 0,
                // 'adhar_no' => isset($post['adhar_no']) ? $post['adhar_no'] : '',
                // 'address' => isset($post['address']) ? $post['address'] : '',
                // 'address_second' => isset($post['address_second']) ? $post['address_second'] : '',
                'employee_no' => isset($post['employee_no']) ? $post['employee_no'] : '',
                // 'auth_no' => isset($post['auth_no']) ? $post['auth_no'] : '',
                ...
                'ins_authorization_no' => isset($post['auth_no']) ? $post['auth_no'] : '',
                'auth_issue_date' => isset($post['auth_issue_date']) ? $post['auth_issue_date'] : '',
                ...
                'subsidy_created' => isset($post['subsidy_created']) ? $post['subsidy_created'] : '',
                ...
                // 'room_no_id' => isset($post['room_no_id']) ? $post['room_no_id'] : '',
                ...
                'iol_type' => isset($post['iol_type']) ? json_encode($post['iol_type']) : '', // Assuming iol_re is an array
                'brand' => isset($post['brand']) ? $post['brand'] : '',
                'corporate_id' => isset($post['corporate_id']) ? $post['corporate_id'] : '',
                'corporate_full_facility' => isset($post['corporate_full_facility']) ? $post['corporate_full_facility'] : 'no',
                'operator' => isset($post['operator']) ? $post['operator'] : '',
                // 'total_amount' => isset($post['total_amount']) ? (float)$post['total_amount'] : 0.00,
                // 'balance' => isset($post['balance']) ? (float)$post['balance'] : 0.00,
            );
                        ...
                        if ($interval->h <= 23 && $interval->h > 0 && ($current_time < $time_duration)) {
                            ...
                            while ($i <= $days) {
                                if (!empty($data_query['end_date']) && $data_query['end_date'] != '') {
                                    ...
                                    $insert_charge_entry = array(
                                        'branch_id' => $users_data['parent_id'],
                                        'ipd_id' => $ipd_id,
                                        'patient_id' => $patient_id,
                                        'type' => 3,
                                        'room_id' => $post['room_id'],
                                        'particular_code' => $data_query['particular_code'],
                                        'group_code' => $data_query['group_code'],
                                        'group_name' => $data_query['group_name'],
                                        'particular_id' => $data_query['particular_id'],
                                        'particular' => $data_query['particular'],
                                        'start_date' => $data_query['start_date'],
                                        'end_date' => $data_query['start_date'],
                                        'quantity' => $data_query['quantity'],
                                        'price' => $data_query['price'],
                                        'panel_price' => $data_query['panel_price'],
                                        'panel_price' => $data_query['panel_price'],
                                        'net_price' => $data_query['net_price'],
                                        'created_date' => $data_query['created_date'],
                                        'is_deleted' => $data_query['is_deleted'],
                                        'status' => $data_query['status'],
                                        'created_by' => $data_query['created_by']
                                    );
