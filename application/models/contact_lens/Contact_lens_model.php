...
class Contact_lens_model extends CI_Model
{
	...
	private function _get_datatables_query()
	{
		...
		if (!empty($search)) {
			...
			if (isset($search['search_type']) && $search['search_type'] != "") {
				$this->db->where('hms_contact_lens.status', $search['search_type']);
			}
	...
	public function save()
	{
		...
		if (!empty($data_id) && $data_id > 0) {
			...
			if(!empty($post['send_to_type'] === 'Contact Lens' ) || $post['mod_type'] === 'help_desk'){
				// Insert a new record
				if (isset($post['contact_lens_items']) && !empty($post['contact_lens_items'])) {
					$this->insert_items($data_id, $post['contact_lens_items'], $user_data, $post);
				}
	...
	public function get_booking_status($patient_id) {
        $this->db->select('cont_lens_status');
    ...
    public function book_patient($patient_id) {
        ...
        $data = ['cont_lens_status' => 1]; // Assuming 1 means booked
        $this->db->where('id', $patient_id);
	...
	{
		$this->db->set('hms_patient.cont_lens_status', 0);
    ...
    public function patient_exists($patient_id = "")
	{
		...
		$user_data = $this->session->userdata('auth_users');
		$this->db->select('hms_contact_lens.id as con_len_id*, hms_patient.id,hms_patient.patient_name');

		$this->db->from('hms_contact_lens');
		$this->db->join('hms_patient', 'hms_patient.id = hms_contact_lens.patient_id');
		$this->db->where('hms_contact_lens.branch_id', $user_data['parent_id']);
		$this->db->where('hms_contact_lens.patient_id', $patient_id);
		$this->db->where('hms_contact_lens.is_deleted', '0');
		$query = $this->db->get();
