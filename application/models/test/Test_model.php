...
class Test_model extends CI_Model
{
  ...
  var $column = array(
    ...
    'hms_insurance_company.insurance_company',
    'hms_insurance_type.insurance_type',
    'hms_patient.polocy_no',
    'hms_patient.tpa_id',
    'hms_patient.ins_amount',
    'hms_patient.ins_authorization_no',
    'hms_patient.gender',
    'hms_patient.age_y',
    'hms_patient.address',
    'path_test_booking.form_f',
    'path_test_booking.tube_no',

    'hms_doctors.doctor_name',
    'hms_employees.name',
    'emp.name',
    'path_test_booking.booking_date',
    'path_test_booking.remarks',
    'hms_payment_mode.payment_mode',
    'path_test_booking.total_amount',
    'path_test_booking.home_collection_amount',
    'path_test_booking.discount',
    'path_test_booking.net_amount',
    'path_test_booking.paid_amount',
    'path_test_booking.balance',
    'path_test_booking.status',
    'path_test_booking.modified_date',
    'path_test_booking.created_by'
  );
  var $order = array('id' => 'desc');
  // added By Nitin Sharma 02/02/2024
  var $test_list_column = array(
    'path_test_booking.id',
    'hms_patient.patient_code',
    'path_test_booking.lab_reg_no',
    'hms_patient.patient_name',
...
(select sum(credit)-sum(debit) as balance from hms_payment as  payment where payment.parent_id = path_test_booking.id AND payment.section_id=1 AND payment.branch_id = " . $users_data['parent_id'] . ") as balance1
    ...
    if (isset($search_date) && !empty($search_date)) {
      if (
        isset($search_date['start_date']) && !empty($search_date['start_date'])
      ) {
      ...
      if (
        isset($search_date['end_date']) && !empty($search_date['end_date'])
      ) {
      ...
      if (
        isset($search_date['profile_id']) && !empty($search_date['profile_id'])
      ) {
        $this->db->where('path_test_booking_to_profile.profile_id = "' . $search_date["profile_id"] . '"');
      }
      if (
        isset($search_date['test_ids']) && !empty($search_date['test_ids'])
      ) {
      ...
      if (
        isset($search_date['dept_id']) && !empty($search_date['dept_id'])
      ) {
...
(select sum(credit)-sum(debit) as balance from hms_payment as  payment where payment.parent_id = path_test_booking.id AND payment.section_id=1 AND payment.branch_id = " . $users_data['parent_id'] . ") as balance1
  ...
  public function get_by_visitor_mobile_no($mobile)
  {
      $users_data = $this->session->userdata('auth_users');
      $this->db->select("hms_visitor.*");
      $this->db->from('hms_visitor');
      $this->db->where('hms_visitor.mobile_no', $mobile);
      $this->db->where('hms_visitor.branch_id', $users_data['parent_id']);
      // $this->db->where('hms_visitor.is_deleted','0');
      $this->db->join('hms_users', 'hms_users.parent_id=hms_visitor.id AND hms_users.users_role=4', 'left');
      // $this->db->join('hms_religion', 'hms_religion.id=hms_visitor.religion_id', 'left');
      // $this->db->join('hms_relation', 'hms_relation.id=hms_visitor.relation_id', 'left');
      // $this->db->join('hms_countries', 'hms_countries.id=hms_visitor.country_id', 'left');
      // $this->db->join('hms_cities', 'hms_cities.id=hms_visitor.city_id', 'left');
      // $this->db->join('hms_state', 'hms_state.id=hms_visitor.state_id', 'left');
      // $this->db->join('hms_simulation', 'hms_simulation.id=hms_visitor.simulation_id', 'left');
      // $this->db->join('hms_gardian_relation', 'hms_gardian_relation.id=hms_visitor.relation_type', 'left');
      // $this->db->join('hms_simulation as hms_sim', 'hms_sim.id=hms_visitor.relation_simulation_id', 'left');
      // $this->db->join('hms_insurance_type', 'hms_insurance_type.id=hms_visitor.insurance_type_id', 'left');
      // $this->db->join('hms_insurance_company', 'hms_insurance_company.id=hms_visitor.ins_company_id', 'left');
  
      $query = $this->db->get();
  
      // Check if query executed successfully
      if (!$query) {
          $error = $this->db->error();  // Fetch the error
          print_r($error);  // Display the error message
