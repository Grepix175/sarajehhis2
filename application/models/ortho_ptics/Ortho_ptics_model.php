...
class Ortho_ptics_model extends CI_Model
{
    ...
    private function _get_datatables_query()
    {
        ...
        $this->db->select("hms_ortho_ptics.id as refraction_id, hms_ortho_ptics.*, hms_patient.*, hms_patient_category.patient_category as patient_category_name, hms_opd_booking.*, hms_doctors.doctor_name, hms_opd_booking.token_no as token, hms_patient.emergency_status, hms_ortho_ptics.created_date as created,hms_ortho_ptics.status as ortho_status");
        ...
        if (!empty($search)) {
            ...
            if (isset($search['search_type']) && $search['search_type'] != "") {
				$this->db->where('hms_ortho_ptics.status', $search['search_type']);
			}
...
// die;
    ...
    public function save($data)
    {
        ...
        if (!empty($data['id'])) {
            ...
            if (!empty($post['send_to_type'] === 'Ortho Paedic') || $post['mod_type'] === 'help_desk') {
                // Insert a new record
    ...
    public function patient_exists($patient_id = "")
	{
		...
		$user_data = $this->session->userdata('auth_users');
		$this->db->select('hms_ortho_ptics.id as ortho_pati_id*, hms_patient.id,hms_patient.patient_name');

		$this->db->from('hms_ortho_ptics');
		$this->db->join('hms_patient', 'hms_patient.id = hms_ortho_ptics.patient_id');
		$this->db->where('hms_ortho_ptics.branch_id', $user_data['parent_id']);
		$this->db->where('hms_ortho_ptics.patient_id', $patient_id);
		$this->db->where('hms_ortho_ptics.is_deleted', '0');
		$query = $this->db->get();
		return $query->row_array();
