...
class Department_master_model extends CI_Model
{
	...
	private function _get_datatables_query()
	{
		...
		$this->db->select("hms_department_master.*");
		$this->db->from($this->table);
		$this->db->where('hms_department_master.is_deleted', '0');

		$this->db->where('hms_department_master.branch_id', $users_data['parent_id']);
	...
	{
		$this->db->select('hms_department_master.*');
		$this->db->from('hms_department_master');
		$this->db->where('hms_department_master.department_id', $id);
		$this->db->where('hms_department_master.is_deleted', '0');
		$query = $this->db->get();
	...
	public function save()
	{
		...
		if (!empty($post['data_id']) && $post['data_id'] > 0) {
			...
			$this->db->where('department_id', $post['data_id']);
			$this->db->update('hms_department_master', $data);
		} else {
			$this->db->set('created_by', $user_data['id']);
			$this->db->set('created_date', date('Y-m-d H:i:s'));
			$this->db->insert('hms_department_master', $data);
		}
	}
	public function delete($id = "")
	{
		if (!empty($id) && $id > 0) {
			...
			$this->db->update('hms_department_master');
		}
	}
	...
	public function deleteall($ids = array())
	{
		if (!empty($ids)) {
			...
			$this->db->set('deleted_by', $user_data['id']);
			$this->db->set('deleted_date', date('Y-m-d H:i:s'));
			$this->db->where('department_id IN (' . $subsidy_ids . ')');
			$this->db->update('hms_department_master');
