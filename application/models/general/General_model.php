...
class General_model extends CI_Model
{
    ...
    public function specialization_list($branch_id = "")
    {
        ...
        if (
            in_array('248', $users_data['permission']['section']) || in_array('224', $users_data['permission']['section']) || in_array('225', $users_data['permission']['section']) || in_array('226', $users_data['permission']['section']) ||
        ...
        /* gyne*/
        if (
            in_array('299', $users_data['permission']['section']) || in_array('300', $users_data['permission']['section']) || in_array('301', $users_data['permission']['section'])
    ...
    public function emg_reg_charge_list()
    {
        $users_data = $this->session->userdata('auth_users');
        $this->db->select('id, charge, status');
        $this->db->where('status', '1');
        $this->db->where('is_deleted', '0');
        $this->db->order_by('charge', 'ASC');
        $this->db->where('is_deleted', 0);
        $this->db->where('branch_id', $users_data['parent_id']);
    ...
    public function doctors_list($doctor_id = '', $branch_id = '')
    {
        ...
        if (!$query) {
            // Print the error
            $error = $this->db->error(); // get the error message and code
            print_r($error);
            // or use this to debug the last query
            echo $this->db->last_query();
            return false;
        }
    ...
    public function total_eme_booking($parent_id = "", $prefix = '')
    {
        $users_data = $this->session->userdata('auth_users');
        $this->db->select('hms_emergency_booking.*');
        $this->db->where('hms_emergency_booking.branch_id', $parent_id);
        //$this->db->where('branch_id',$users_data['parent_id']);
        // $this->db->where('is_deleted !=2');
        // $this->db->where('type',2);
        if (!empty($prefix)) {
            $this->db->where('hms_emergency_booking.created_date >= (select created_date from hms_branch_unique_ids where unique_id = "9" AND  branch_id = "' . $parent_id . '")');
        }
...
*/
    ...
    public function check_receipt_number($section_id = "", $parent_id = "")
    {
        $users_data = $this->session->userdata('auth_users');
        $total = '1';
        if ($section_id == '1' || $section_id == '8') {   //opd
            $this->db->select('*');
            $this->db->where('branch_id', $users_data['parent_id']);
            ...
            //echo $this->db->last_query(); die;
        } elseif ($section_id == '2' || $section_id == '12') {   //opd billing
            $this->db->select('*');
            $this->db->where('branch_id', $users_data['parent_id']);
            $this->db->where('id', $parent_id);
            $this->db->where('is_deleted!=2');
        ...
        {   //opd billing
            ...
            $total = $query->num_rows();
            //echo $this->db->last_query(); die;
        } elseif ($section_id == '4' || $section_id == '11') {   //opd billing
            $this->db->select('*');
            $this->db->where('branch_id', $users_data['parent_id']);
            $this->db->where('id', $parent_id);
            $this->db->where('is_deleted!=2');
            ...
            $total = $query->num_rows();
            //echo $this->db->last_query(); die;
        } elseif ($section_id == '5' || $section_id == '13') {   //vaccination
            $this->db->select('*');
            $this->db->where('branch_id', $users_data['parent_id']);
            $this->db->where('id', $parent_id);
            $this->db->where('is_deleted!=2');
            ...
            $total = $query->num_rows();
            //echo $this->db->last_query(); die;
        } elseif ($section_id == '6' || $section_id == '10') {   //medicine
            $this->db->select('*');
            $this->db->where('branch_id', $users_data['parent_id']);
            $this->db->where('id', $parent_id);
            $this->db->where('is_deleted!=2');
            ...
            $total = $query->num_rows();
            //echo $this->db->last_query(); die;
        } elseif ($section_id == '15' || $section_id == '16') {   //medicine
            $this->db->select('*');
            $this->db->where('branch_id', $users_data['parent_id']);
            $this->db->where('id', $parent_id);
            $this->db->where('is_deleted!=2');
