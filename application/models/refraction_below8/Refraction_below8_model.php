...
class Refraction_below8_model extends CI_Model
{
	...
	private function _get_datatables_query()
	{
		...
		$this->db->select("hms_std_eye_prescription.*, hms_patient.simulation_id, hms_patient.patient_name,hms_patient.pat_status,hms_patient.emergency_status,hms_patient.gender, hms_patient.patient_code, hms_patient.mobile_no, hms_patient.age_y, hms_patient.age_m, hms_patient.age_d, hms_opd_booking.dilate_status, hms_opd_booking.app_type, hms_opd_booking.token_no");
	...
	public function get_booking_status($patient_id) {
        $this->db->select('ref_below_status');
    ...
    public function book_patient($patient_id) {
        ...
        $data = ['ref_below_status' => 1]; // Assuming 1 means booked
        $this->db->where('id', $patient_id);
	...
	{
		$this->db->set('hms_patient.ref_below_status', 0);
    ...
    public function patient_exists($patient_id = "")
	{
		...
		$user_data = $this->session->userdata('auth_users');
		$this->db->select('hms_std_eye_prescription.id as std_ref_below_id*, hms_patient.id,hms_patient.patient_name');
