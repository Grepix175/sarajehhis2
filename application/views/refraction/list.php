  ...
  <script type="text/javascript">
    ...
    $(document).ready(function () {
      ...
      $('#adv_search').on('click', function () {
        $modal.load('<?php echo base_url() . 'refraction/advance_search/' ?>',
          {
          },
          function () {
            $modal.modal('show');
  ...
  <a href="javascript:void(0)" class="btn-a-search" id="patient_adv_search">
        ...
        <div class="btns">
          <?php if (in_array('2486', $users_data['permission']['action'])) { ?>
            <!-- <button class="btn-update" id="1modal_add"
                onclick="window.location.href='<?php echo base_url('refraction/add'); ?>'">
              <i class="fa fa-plus"></i> New
            </button> -->
          <?php } ?>
          <a data-toggle="tooltip" title="Download list in excel" href="#" id="refraction_download_excel"
            class="btn-anchor m-b-2">
            <i class="fa fa-file-excel-o"></i> Excel
          </a>
          <a data-toggle="tooltip" title="Download list in pdf" href="#" id="refraction_download_pdf"
            class="btn-anchor m-b-2">
    ...
    <script>
      ...
      function reset_search() {
        ...
        $.ajax({
          url: "<?php echo base_url(); ?>refraction/reset_search/",
          success: function (result) {
            reload_table();
          }
        });
      ...
      $("#search_form").on("submit", function (event) {
        ...
        $.ajax({
          url: "<?php echo base_url('refraction/advance_search/'); ?>",
          type: "post",
          data: $(this).serialize(),
          success: function (result) {
            reload_table();
      ...
      function delete_vision(rate_id) {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('refraction/delete/'); ?>" + rate_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
      ...
      document.getElementById('refraction_download_excel').addEventListener('click', function (e) {
        e.preventDefault();
        ...
        var url = '<?php echo base_url("refraction/refraction_excel"); ?>';
      ...
      document.getElementById('refraction_download_pdf').addEventListener('click', function (e) {
        e.preventDefault();
        ...
        var url = '<?php echo base_url("refraction/refraction_pdf"); ?>';
