...
<head>
  ...
  <script type="text/javascript">
    ...
    if (in_array('2485', $users_data['permission']['action'])) {
      ...
      $(document).ready(function () {
        table = $('#table').DataTable({
          ...
          "pageLength": '20',
          "ajax": {
            "url": "<?php echo base_url('emg_reg_charge/ajax_list') ?>",
            "type": "POST",
    ...
    $(document).ready(function () {
      var $modal = $('#load_add_corporate_modal_popup');
      $('#modal_add').on('click', function () {
        $modal.load('<?php echo base_url() . 'emg_reg_charge/add/' ?>',
          {
            //'id1': '1',
            //'id2': '2'
          },
    ...
    function edit_patient_category(id) {
      var $modal = $('#load_add_corporate_modal_popup');
      $modal.load('<?php echo base_url() . 'emg_reg_charge/edit/' ?>' + id,
        {
          //'id1': '1',
          //'id2': '2'
        },
    ...
    function view_patient_category(id) {
      var $modal = $('#load_add_corporate_modal_popup');
      $modal.load('<?php echo base_url() . 'emg_reg_charge/view/' ?>' + id,
        {
          //'id1': '1',
          //'id2': '2'
        },
      ...
      if (allVals != "") {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              type: "POST",
              url: "<?php echo base_url('emg_reg_charge/deleteall'); ?>",
              data: { row_id: allVals },
...
<body>
  ...
  <div class="container-fluid">
    ...
    <script>
      ...
      function delete_patient_category(rate_id) {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('emg_reg_charge/delete/'); ?>" + rate_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
      ...
      $(document).ready(function () {
        $('#load_add_corporate_modal_popup').on('shown.bs.modal', function (e) {
          $('.inputFocus').focus();
        })
      });  
    </script>
    ...
    <div id="load_add_corporate_modal_popup" class="modal fade" role="dialog" data-backdrop="static"
