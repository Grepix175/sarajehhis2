    ...
    if (in_array('2485', $users_data['permission']['action'])) {
      ...
      $(document).ready(function () {
        table = $('#table').DataTable({
          ...
          "order": [],
          "ajax": {
            "url": "<?php echo base_url('emg_reg_charge/archive_ajax_list') ?>",
            "type": "POST",
            "data": function (d) {
      ...
      if (allVals != "") {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              type: "POST",
              url: "<?php echo base_url('emg_reg_charge/restoreall'); ?>",
              data: { row_id: allVals },
    ...
    <section class="userlist">
      ...
      <div class="userlist-right">
        <div class="btns">
          ...
          <?php } ?>
          <button class="btn-update" onclick="window.location.href='<?php echo base_url('emg_reg_charge'); ?>'">
    ...
    <script>
      ...
      function restore_patient_category(employee_type_id) {
        ...
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('emg_reg_charge/restore/'); ?>" + employee_type_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
      ...
      function trash(patient_category_id) {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('emg_reg_charge/trash/'); ?>" + patient_category_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
        ...
        if (allVals != "") {
          $('#confirm').modal({
            ...
            .one('click', '#delete', function (e) {
              $.ajax({
                type: "POST",
                url: "<?php echo base_url('emg_reg_charge/trashall'); ?>",
                data: { row_id: allVals },
                success: function (result) {
                  flash_session_msg(result);
                  reload_table();
