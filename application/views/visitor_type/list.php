...
<head>
  ...
  <script type="text/javascript">
    ...
    if (in_array('2485', $users_data['permission']['action'])) {
      ...
      $(document).ready(function () {
        table = $('#table').DataTable({
          ...
          "pageLength": '20',
          "ajax": {
            "url": "<?php echo base_url('visitor_type/ajax_list') ?>",
            "type": "POST",
    ...
    $(document).ready(function () {
      var $modal = $('#load_add_visitor_type_modal_popup');
      $('#modal_add').on('click', function () {
        $modal.load('<?php echo base_url() . 'visitor_type/add/' ?>',
          {
            //'id1': '1',
            //'id2': '2'
          },
    ...
    function edit_patient_category(id) {
      var $modal = $('#load_add_visitor_type_modal_popup');
      $modal.load('<?php echo base_url() . 'visitor_type/edit/' ?>' + id,
        {
          //'id1': '1',
          //'id2': '2'
        },
    ...
    function view_patient_category(id) {
      var $modal = $('#load_add_visitor_type_modal_popup');
      $modal.load('<?php echo base_url() . 'visitor_type/view/' ?>' + id,
        {
          //'id1': '1',
          //'id2': '2'
        },
      ...
      if (allVals != "") {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              type: "POST",
              url: "<?php echo base_url('visitor_type/deleteall'); ?>",
              data: { row_id: allVals },
...
<body>
  ...
  <div class="container-fluid">
        ...
        <div class="btns">
          ...
          <?php if (in_array('2489', $users_data['permission']['action'])) {
            ...
            <button class="btn-update"
              onclick="window.location.href='<?php echo base_url('visitor_type/archive'); ?>'">
    ...
    <script>
      ...
      function delete_patient_category(rate_id) {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('visitor_type/delete/'); ?>" + rate_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
      ...
      $(document).ready(function () {
        $('#load_add_visitor_type_modal_popup').on('shown.bs.modal', function (e) {
          $('.inputFocus').focus();
        })
      });  
    </script>
    ...
    <div id="load_add_visitor_type_modal_popup" class="modal fade" role="dialog" data-backdrop="static"
