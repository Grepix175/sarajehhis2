    ...
    if (in_array('2485', $users_data['permission']['action'])) {
      ...
      $(document).ready(function () {
        table = $('#table').DataTable({
          ...
          "order": [],
          "ajax": {
            "url": "<?php echo base_url('visitor_type/archive_ajax_list') ?>",
            "type": "POST",
            "data": function (d) {
      ...
      if (allVals != "") {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              type: "POST",
              url: "<?php echo base_url('visitor_type/restoreall'); ?>",
              data: { row_id: allVals },
    ...
    <section class="userlist">
      ...
      <div class="userlist-right">
        <div class="btns">
          ...
          <?php } ?>
          <button class="btn-update" onclick="window.location.href='<?php echo base_url('visitor_type'); ?>'">
    ...
    <script>
      ...
      function restore_patient_category(employee_type_id) {
        ...
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('visitor_type/restore/'); ?>" + employee_type_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
      ...
      function trash(patient_category_id) {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('visitor_type/trash/'); ?>" + patient_category_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
        ...
        if (allVals != "") {
          $('#confirm').modal({
            ...
            .one('click', '#delete', function (e) {
              $.ajax({
                type: "POST",
                url: "<?php echo base_url('visitor_type/trashall'); ?>",
                data: { row_id: allVals },
                success: function (result) {
                  flash_session_msg(result);
                  reload_table();
