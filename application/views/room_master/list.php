...
<head>
  ...
  <script type="text/javascript">
    ...
    if (in_array('347', $users_data['permission']['action'])) {
      ...
      $(document).ready(function () {
        table = $('#table').DataTable({
          ...
          "pageLength": '20',
          "ajax": {
            "url": "<?php echo base_url('room_master/ajax_list') ?>",
            "type": "POST",
          },
          "columnDefs": [
            {
    ...
    function edit_medicine_unit(id) {
      var $modal = $('#load_add_medicine_unit_modal_popup');
      $modal.load('<?php echo base_url() . 'room_master/edit/' ?>' + id,
        {
          //'id1': '1',
          //'id2': '2'
        },
      ...
      if (allVals != "") {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              type: "POST",
              url: "<?php echo base_url('room_master/deleteall'); ?>",
              data: { row_id: allVals },
      ...
      <div class="userlist-right">
        <div class="btns">
          ...
          if (in_array('416', $users_data['permission']['action'])) {
            ?>
            <button class="btn-exit" onclick="window.location.href='<?php echo base_url('room_master/archive'); ?>'">
    ...
    <script>
      function delete_medicine_unit(rate_id) {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('room_master/delete/'); ?>" + rate_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
