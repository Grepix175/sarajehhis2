...
<head>
  ...
  <script type="text/javascript">
    ...
    if (in_array('416', $users_data['permission']['action'])) {
      ...
      $(document).ready(function () {
        table = $('#table').DataTable({
          ...
          "order": [],
          "ajax": {
            "url": "<?php echo base_url('room_master/archive_ajax_list') ?>",
            "type": "POST",
          },
          "columnDefs": [
            {
    ...
    function view_employee(id) {
      var $modal = $('#load_add_modal_popup');
      $modal.load('<?php echo base_url() . 'room_master/view/' ?>' + id,
        {
          //'id1': '1',
          //'id2': '2'
        },
      ...
      if (allVals != "") {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              type: "POST",
              url: "<?php echo base_url('room_master/restoreall'); ?>",
              data: { row_id: allVals },
    ...
    <section class="userlist">
      <div class="userlist-box">
        ...
        <form>
          ...
          if (in_array('416', $users_data['permission']['action'])) {
            ...
            <table id="table" class="table table-striped table-bordered table-responsive medicine_unit_archive"
      ...
      <div class="userlist-right">
        <div class="btns">
          ...
          <?php } ?>
          <button class="btn-exit" onclick="window.location.href='<?php echo base_url('room_master'); ?>'">
    ...
    <script>
      function restore_medicine_unit(employee_type_id) {
        ...
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('room_master/restore/'); ?>" + employee_type_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
      ...
      function trash(unit_id) {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('room_master/trash/'); ?>" + unit_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
        ...
        if (allVals != "") {
          $('#confirm').modal({
            ...
            .one('click', '#delete', function (e) {
              $.ajax({
                type: "POST",
                url: "<?php echo base_url('room_master/trashall'); ?>",
                data: { row_id: allVals },
                success: function (result) {
                  flash_session_msg(result);
                  reload_table();
