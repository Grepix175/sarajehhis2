    ...
    if (in_array('371', $users_data['permission']['action'])) {
      ...
      $(document).ready(function () {
        table = $('#table').DataTable({
          ...
          "order": [],
          "ajax": {
            "url": "<?php echo base_url('hospital_code_entry/archive_ajax_list') ?>",
            "type": "POST",
          },
          "columnDefs": [
            {
      ...
      if (allVals != "") {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              type: "POST",
              url: "<?php echo base_url('hospital_code_entry/restoreall'); ?>",
              data: { row_id: allVals },
      ...
      <div class="userlist-box">
        ...
        <form>
          <?php if (in_array('371', $users_data['permission']['action'])) {
            ...
            <table id="table" class="table table-striped table-bordered table-responsive medicine_entry_archive"
    ...
    <script>
      function restore_medicine_entry(rate_id) {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('hospital_code_entry/restore/'); ?>" + rate_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
      ...
      function trash(id) {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('hospital_code_entry/trash/'); ?>" + id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
        ...
        if (allVals != "") {
          $('#confirm').modal({
            ...
            .one('click', '#delete', function (e) {
              $.ajax({
                type: "POST",
                url: "<?php echo base_url('hospital_code_entry/trashall'); ?>",
                data: { row_id: allVals },
                success: function (result) {
                  flash_session_msg(result);
                  reload_table();
