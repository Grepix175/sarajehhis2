    ...
    <?php if (in_array('118', $users_data['permission']['action'])): ?>
      $(document).ready(function () {
        table = $('#table').DataTable({
          ...
          "order": [],
          "ajax": {
            "url": "<?php echo base_url('visitor/archive_ajax_list') ?>",
            "type": "POST",
          },
          "columnDefs": [
            {
      ...
      if (allVals != "") {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              type: "POST",
              url: "<?php echo base_url('visitor/restoreall'); ?>",
              data: { row_id: allVals },
      ...
      <div class="userlist-box">
        ...
        <form>
          <?php if (in_array('118', $users_data['permission']['action'])): ?>
            ...
            <table id="table" class="table table-striped table-bordered table-responsive patient_list_archive"
      ...
      <div class="userlist-right relative">
        <div class="fixed">
          <div class="btns">
            ...
            <?php endif; ?>
            <button class="btn-exit" onclick="window.location.href='<?php echo base_url('visitor'); ?>'">
    ...
    <script>
      function restore_patient(rate_id) {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('visitor/restore/'); ?>" + rate_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
        ...
        if (allVals != "") {
          $('#confirm').modal({
            ...
            .one('click', '#delete', function (e) {
              $.ajax({
                type: "POST",
                url: "<?php echo base_url('visitor/trashall'); ?>",
                data: { row_id: allVals },
                success: function (result) {
                  flash_session_msg(result);
                  reload_table();
      ...
      function trash(doctor_id) {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('visitor/trash/'); ?>" + doctor_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
