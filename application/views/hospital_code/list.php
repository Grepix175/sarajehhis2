  ...
  <script type="text/javascript">
    ...
    if (in_array('347', $users_data['permission']['action'])) {
      ...
      $(document).ready(function () {
        table = $('#table').DataTable({
          ...
          "pageLength": '20',
          "ajax": {
            "url": "<?php echo base_url('hospital_code/ajax_list') ?>",
            "type": "POST",
          },
          "columnDefs": [
            {
    ...
    $(document).ready(function () {
      ...
      $('#modal_add').on('click', function () {
        $modal.load('<?php echo base_url() . 'hospital_code/add/' ?>',
          {
            //'id1': '1',
            //'id2': '2'
          },
    ...
    function edit_medicine_unit(id) {
      var $modal = $('#load_add_medicine_unit_modal_popup');
      $modal.load('<?php echo base_url() . 'hospital_code/edit/' ?>' + id,
        {
          //'id1': '1',
          //'id2': '2'
        },
    ...
    function view_medicine_unit(id) {
      var $modal = $('#load_add_medicine_unit_modal_popup');
      $modal.load('<?php echo base_url() . 'hospital_code/view/' ?>' + id,
        {
          //'id1': '1',
          //'id2': '2'
        },
      ...
      if (allVals != "") {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              type: "POST",
              url: "<?php echo base_url('hospital_code/deleteall'); ?>",
              data: { row_id: allVals },
      ...
      <div class="userlist-box">
        ...
        <form>
          ...
          if (in_array('347', $users_data['permission']['action'])) {
            ...
            <table id="table" class="table table-striped table-bordered table-responsive medicine_unit_list"
      ...
      <div class="userlist-right">
        <div class="btns">
          ...
          if (in_array('416', $users_data['permission']['action'])) {
            ?>
            <button class="btn-exit" onclick="window.location.href='<?php echo base_url('hospital_code/archive'); ?>'">
    ...
    <script>
      function delete_medicine_unit(rate_id) {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('hospital_code/delete/'); ?>" + rate_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
