  ...
  <script type="text/javascript">
    ...
    if (in_array('2413', $users_data['permission']['action'])) {
      ...
      $(document).ready(function () {
        table = $('#table').DataTable({
            ...
            "pageLength": '20',
            "ajax": {
                "url": "<?php echo base_url('hess_chart/ajax_list') ?>",
                "type": "POST",
            },
            "columnDefs": [
                {
            ...
            "createdRow": function (row, data, dataIndex) {
                // Access emergency_status value (assuming it's the last column or as needed)
                var emergencyStatus = data[data.length - 1]; // Adjust this if your status is in another column

                // Get the first column cell (adjust the index as needed)
                var firstColumn = $('td', row).eq(1); // Adjust the column index as needed
                ...
                if (emergencyStatus == 1) {
                    ...
                    firstColumn.css('background-color', 'white'); // Default color
                }
            }
        });
    });
    ...
    $(document).ready(function () {
      ...
      $('#adv_search').on('click', function () {
        $modal.load('<?php echo base_url() . 'hess_chart/advance_search/' ?>',
          {
          },
          function () {
            $modal.modal('show');
    ...
    function edit_prescription(id) {
      var $modal = $('#load_add_modal_popup');
      $modal.load('<?php echo base_url() . 'hess_chart/edit/' ?>' + id,
        {
          //'id1': '1',
          //'id2': '2'
        },
    ...
    function view_prescription(id) {
      var $modal = $('#load_add_modal_popup');
      $modal.load('<?php echo base_url() . 'hess_chart/view/' ?>' + id,
        {
          //'id1': '1',
          //'id2': '2'
        },
      ...
      if (allVals != "") {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              type: "POST",
              url: "<?php echo base_url('hess_chart/deleteall'); ?>",
              data: { row_id: allVals },
  ...
  <div class="container-fluid">
    ...
    <script>
      ...
      function reset_search() {
        ...
        $.ajax({
          url: "<?php echo base_url(); ?>hess_chart/reset_search/",
          success: function (result) {
            reload_table();
          }
        });
      ...
      $("#search_form").on("submit", function (event) {
        ...
        $.ajax({
          url: "<?php echo base_url('hess_chart/advance_search/'); ?>",
          type: "post",
          data: $(this).serialize(),
          success: function (result) {
            reload_table();
        ...
        <?php if (isset($_GET['status']) && $_GET['status'] == 'print' && !isset($_GET['type'])) { ?>
          $('#confirm_print').modal({
            ...
            .one('click', '#cancel', function (e) {
              window.location.href = '<?php echo base_url('hess_chart'); ?>';
            });
      ...
      var url = '<?php echo base_url("hess_chart/help_desk_excel"); ?>';
    ...
    document.getElementById('help_desk_download_pdf').addEventListener('click', function (e) {
      ...
      var url = '<?php echo base_url("hess_chart/help_desk_pdf"); ?>';
        ...
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <a data-dismiss="modal" class="btn-anchor"
              onClick="return print_window_page('<?php echo base_url("hess_chart/print_prescriptions"); ?>');">Print</a>
