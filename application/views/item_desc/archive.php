...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('item_desc/archive_ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('item_desc/restoreall');?>",
                      data: {row_id: allVals},
  	...
  	<div class="userlist-right">
        ...
        <?php
         if(in_array('416',$users_data['permission']['action'])) {
          ...
          <?php } ?> 
        <button class="btn-exit" onclick="window.location.href='<?php echo base_url('item_desc'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('item_desc/restore/'); ?>"+employee_type_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {     
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('item_desc/trash/'); ?>"+unit_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('item_desc/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
