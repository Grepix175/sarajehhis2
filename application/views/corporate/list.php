...
<head>
  ...
  <script type="text/javascript">
    ...
    if (in_array('2485', $users_data['permission']['action'])) {
      ...
      $(document).ready(function () {
        table = $('#table').DataTable({
          ...
          "pageLength": '20',
          "ajax": {
            "url": "<?php echo base_url('corporate/ajax_list') ?>",
            "type": "POST",
    ...
    function edit_patient_category(id) {
      var $modal = $('#load_add_corporate_modal_popup');
      $modal.load('<?php echo base_url() . 'corporate/edit/' ?>' + id,
        {
          //'id1': '1',
          //'id2': '2'
        },
      ...
      if (allVals != "") {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              type: "POST",
              url: "<?php echo base_url('corporate/deleteall'); ?>",
              data: { row_id: allVals },
    ...
    <script>
      ...
      function delete_patient_category(rate_id) {
        $('#confirm').modal({
          ...
          .one('click', '#delete', function (e) {
            $.ajax({
              url: "<?php echo base_url('corporate/delete/'); ?>" + rate_id,
              success: function (result) {
                flash_session_msg(result);
                reload_table();
              }
