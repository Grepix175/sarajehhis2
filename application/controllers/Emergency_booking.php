...
class Emergency_booking extends CI_Controller
{
  ...
  public function index()
  {
    ...
    $data['page_title'] = 'Emergency Booking List';
    ...
    $this->load->view('emergency_booking/list', $data);
  }
  ...
  public function ajax_list()
  {
    ...
    $list = $this->emergency_booking->get_datatables();
    // echo "<pre>";
    // print_r($list); die;
    $data = array();
    $no = $_POST['start'];
    ...
    foreach ($list as $test) {
      $no++;
      $row = array();
      $prescription_count = $this->emergency_booking->get_prescription_count($test->id);
      ////////// Check  List /////////////////
      $check_script = "";
      if ($i == $total_num) {
      ...
      $pat_status = '';
      $patient_status = $this->emergency_booking->get_by_id_patient_details($test->id);
      ...
      {
        $prescription_count_eye = $this->emergency_booking->get_prescription_count_for_eye($test->id);
        ...
        $prescription_count_dental = $this->emergency_booking->get_prescription_count_for_dental($test->id);
        ...
        $prescription_count_gyne = $this->emergency_booking->get_prescription_count_for_gynecology($test->id);
        ...
        if (in_array('308', $users_data['permission']['section'])) {
          $btn_prescription .= '<li><a  href="' . base_url("emergency_booking/partograph_pres/" . $test->id) . '" title="Add Partograph"><i class="fa fa-pencil"></i> Add Partograph</a></li>';
        }
      } else {
        ...
        if ($prescription_count < 1) {
          ...
          if (in_array('532', $users_data['permission']['action'])) {
            $btn_prescription .= '<li><a  href="' . base_url("emergency_booking/prescription/" . $test->id) . '" title="Add Prescription"><i class="fa fa-pencil"></i> Add Prescription</a></li>';
      ...
      $print_pdf_url = "'" . base_url('emergency_booking/print_booking_report/' . $test->id . '/' . $test->branch_id) . "'";

      $print_barcode_url = "'" . base_url('emergency_booking/print_barcode/' . $test->id) . "'";
      ...
      $btn_download_image = '';
      $get_pres = $this->emergency_booking->get_records_for_prescription_id($test->id);
      $flag_id = 0;
      $pres_id = 0;
      if (!empty($get_pres)) {
    ...
    $recordsTotal = $this->emergency_booking->count_all();
    $recordsFiltered = $recordsTotal;
    $output = array(
      "draw" => $_POST['draw'],
      "recordsTotal" => $recordsTotal, //$this->emergency_booking->count_all(),
  ...
  public function download_image($id = "", $branch_id = '')
  {
    ...
    $get_by_id_data = $this->emergency_booking->get_all_detail_print($id, $branch_id);
    ...
    $get_refund = $this->emergency_booking->get_payment_refund($id, $branch_id);
    $refund_payment = 0;
    ...
    $this->load->view('emergency_booking/print_template_opd', $data);
  ...
  {
    $result = $this->emergency_booking->dilated_start($_POST['booked_id']);
  ...
  public function delete_booking($id = "")
  {
    ...
    if (!empty($id) && $id > 0) {
      $result = $this->emergency_booking->delete_booking($id);
  ...
  function deleteall()
  {
    ...
    $post = $this->input->post();
    if (!empty($post)) {
      $result = $this->emergency_booking->deleteall($post['row_id']);
  ...
  public function view($id = "")
  {
    ...
    if (isset($id) && !empty($id) && is_numeric($id)) {
      $data['form_data'] = $this->emergency_booking->get_by_id($id);
      $data['page_title'] = $data['form_data']['doctor_name'] . " detail";
      $this->load->view('emergency_booking/view', $data);
    }
  }
  ...
  public function archive()
  {
    ...
    $this->load->view('emergency_booking/archive', $data);
  }
  ...
  public function archive_ajax_list()
  {
    ...
    $this->load->model('emergency_booking/opd_archive_model', 'opd_archive');
    $users_data = $this->session->userdata('auth_users');
  ...
  {
    $opd_list = $this->emergency_booking->employee_type_list();
  ...
  public function add($pid = "")
  {
    ...
    if (isset($_GET['lid']) && !empty($_GET['lid']) && $_GET['lid'] > 0) {
      $lead_data = $this->emergency_booking->crm_get_by_id($_GET['lid']);
      ...
      $data['specialization_list'] = $this->general_model->specialization_list();
      $data['attended_doctor_list'] = $this->emergency_booking->attended_doctor_list();
    ...
    if (!empty($post['branch_id'])) {
      $data['referal_doctor_list'] = $this->emergency_booking->referal_doctor_list($post['branch_id']);
    } else {
      $data['referal_doctor_list'] = $this->emergency_booking->referal_doctor_list();
    }

    if (!empty($post['branch_id'])) {
      $data['referal_hospital_list'] = $this->emergency_booking->referal_hospital_list($post['branch_id']);
    } else {
      $data['referal_hospital_list'] = $this->emergency_booking->referal_hospital_list();
    }

    if (!empty($post['branch_id'])) {
      $data['attended_doctor_list'] = $this->emergency_booking->attended_doctor_list($post['branch_id'], $post['specialization']);
    } else {
      $data['attended_doctor_list'] = $this->emergency_booking->attended_doctor_list();
    }

    if (!empty($post['branch_id'])) {
      $data['attended_doctor_list'] = $this->emergency_booking->attended_doctor_list($post['branch_id']);
    } else {
      $data['attended_doctor_list'] = $this->emergency_booking->attended_doctor_list();
    }

    if (!empty($post['branch_id'])) {
      $data['employee_list'] = $this->emergency_booking->employee_list($post['branch_id']);
    } else {
      $data['employee_list'] = $this->emergency_booking->employee_list();
    }

    if (!empty($post['branch_id'])) {
      $data['profile_list'] = $this->emergency_booking->profile_list($post['branch_id']);
    } else {
      $data['profile_list'] = $this->emergency_booking->profile_list();
    }
    if (!empty($post['branch_id'])) {
      $data['source_list'] = $this->emergency_booking->source_list($post['branch_id']);

    } else {
      $data['source_list'] = $this->emergency_booking->source_list();

    }

    if (!empty($post['branch_id'])) {
      $data['diseases_list'] = $this->emergency_booking->diseases_list($post['branch_id']);
    } else {
      $data['diseases_list'] = $this->emergency_booking->diseases_list();
    }
    if (!empty($post['branch_id'])) {
    ...
    $data['corporate_list'] = $this->emergency_booking->corporate_list();
    $data['subsidy_list'] = $this->emergency_booking->subsidy_list();
    $data['department_list'] = $this->emergency_booking->department_list();
    ...
    $token_type = $this->emergency_booking->get_token_setting();
    $token_type = $token_type['type'];
    $validity = 0;
    $validity_days = $this->emergency_booking->get_opd_validity_days();
    ...
    $data['page_title'] = "Emergency Booking";
    ...
    if (isset($_GET['lid']) && !empty($_GET['lid']) && $_GET['lid'] > 0 && !empty($opd_attended_doctor)) {
      ...
      $doct_set = $this->emergency_booking->default_doctor_setting();
      $def_specl_id = '';
      $def_doct_id = '';
      if (!empty($doct_set)) {
    ...
    $booking_code = generate_unique_id(76);
    // echo "<pre>";
    // print_r($booking_code);
    // die;
    $data['form_error'] = [];
    ...
    if (isset($post) && !empty($post)) {
      ...
      if ($this->form_validation->run() == TRUE) {
        $booking_id = $this->emergency_booking->save_booking();
        $this->session->set_userdata('opd_booking_id', $booking_id);
        if (!empty($booking_id)) {
          $get_by_id_data = $this->emergency_booking->get_by_id($booking_id);
    ...
    $this->load->view('emergency_booking/emergency_booking', $data);
  }
  ...
  public function edit_booking($id = "")
  {
    ...
    if (isset($id) && !empty($id) && is_numeric($id)) {
      ...
      $data['subsidy_list'] = $this->emergency_booking->subsidy_list();
      $data['department_list'] = $this->emergency_booking->department_list();
      $data['page_title'] = "Update Emergency Booking";
      $post = $this->input->post();
      $data['form_error'] = '';
      $token_type = $this->emergency_booking->get_token_setting();
      $token_type = $token_type['type'];
      ...
      $data['form_data'] = array(
        ...
        'booking_code' => $result['eme_booking_code'],
  ...
  public function generate_token()
  {
    ...
    if (isset($post) && !empty($post) && !empty($post['doctor_id'])) {
      ...
      $token_type_data = $this->emergency_booking->get_token_setting();
      ...
      if ($token_type == '0') //hospital wise token no
      {
        ...
        $patient_token_details_for_hospital = $this->emergency_booking->get_patient_token_details_for_type_hospital($booking_date, $token_type);
      ...
      {
        // echo "hi";die;
        $patient_token_details_for_doctor = $this->emergency_booking->get_patient_token_details_for_type_doctor($doctor_id, $booking_date, $token_type);
      ...
      {
        // echo "hi";die;
        $patient_token_details_for_specialization = $this->emergency_booking->get_patient_token_details_for_type_specialization($specilization_id, $booking_date, $token_type);
        //  print_r($patient_token_details_for_doctor);die;
  ...
  public function confirm_booking($id)
  {
    if (isset($id) && !empty($id) && is_numeric($id)) {
      ...
      $rate_data = $this->emergency_booking->opd_doctor_rate($result['attended_doctor']);
      if (!empty($total_amount) && $total_amount != '0.00') {
        $total_amount = $total_amount;
      } else {
        $total_amount = $rate_data;
      }
      if (!empty($net_amount) && $total_amount != '0.00') {
        $net_amount = $net_amount;
      } else {
        $net_amount = $rate_data;
      }
      if (!empty($paid_amount) && $total_amount != '0.00' && $paid_amount != '0.00') {
        $paid_amount = $paid_amount;
      } else {
        $paid_amount = $rate_data;
      }
      if (!empty($discount)) {
        $discount = $discount;
      } else {
      ...
      if (!empty($post)) {
        ...
        if ($this->form_validation->run() == TRUE) {
          $this->emergency_booking->confirm_booking();
          echo 1;
          return false;
        } else {
          $data['form_error'] = validation_errors();
      ...
      $this->load->view('emergency_booking/confirm', $data);
    }
  }
  ...
  private function _validate()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      $balance_amount = $this->emergency_booking->check_patient_balance($post['patient_id']);
      $insurance_type = '';
      if (isset($post['insurance_type'])) {
        $insurance_type = $post['insurance_type'];
      }
  ...
  public function prescription($booking_id = "")
  {
    ...
    if ($booking_id > 0) {
      $this->load->model('emergency_booking/opd_model');
      $this->load->model('patient/patient_model');
    ...
    $data['examination_list'] = $this->emergency_booking->examinations_list();
    $data['diagnosis_list'] = $this->emergency_booking->diagnosis_list();
    $data['suggetion_list'] = $this->emergency_booking->suggetion_list();
    $data['prv_history'] = $this->emergency_booking->prv_history_list();
    $data['personal_history'] = $this->emergency_booking->personal_history_list();
    $data['template_list'] = $this->emergency_booking->template_list();
    $data['vitals_list'] = $this->general_model->vitals_list();
    ...
    if (isset($post) && !empty($post)) {
      ...
      /*$this->emergency_booking->save_prescription();
      $this->session->set_flashdata('success','Prescription successfully added.');
      redirect(base_url('prescription'));*/

      $prescription_id = $this->emergency_booking->save_prescription();
    ...
    $this->load->view('emergency_booking/prescription', $data);
  }
  ...
  {
    if ($template_id > 0) {
      $templatedata = $this->emergency_booking->template_data($template_id);
      echo $templatedata;
    }
  }
  ...
  {
    if ($template_id > 0) {
      $templatetestdata = $this->emergency_booking->get_template_test_data($template_id);
  ...
  {
    if ($template_id > 0) {
      $templatetestdata = $this->emergency_booking->get_template_prescription_data($template_id);
  ...
  {
    if ($complaints_id > 0) {
      $complaintsname = $this->emergency_booking->compalaints_list($complaints_id);
      echo $complaintsname;
    }
  }
  ...
  {
    if ($examination_id > 0) {
      $examination_name = $this->emergency_booking->examination_list($examination_id);
  ...
  {
    if ($diagnosis_id > 0) {
      $diagnosis_name = $this->emergency_booking->diagnosis_name($diagnosis_id);
      echo $diagnosis_name;
    }
  }
  //hms_opd_suggetion
  ...
  {
    if ($suggetion_id > 0) {
      $suggetion_name = $this->emergency_booking->suggetion_name($suggetion_id);
      echo $suggetion_name;
    }
  }
  ...
  function personal_history_name($personal_his_id = "")
  {
    if ($personal_his_id > 0) {
      $personal_his_name = $this->emergency_booking->personal_history_name($personal_his_id);
  ...
  {
    if ($pre_id > 0) {
      $pre_name = $this->emergency_booking->prv_history_name($pre_id);
      echo $pre_name;
    }
  }
  ...
  public function particular_billing_list()
  {
    ...
    $this->load->view('emergency_booking/particular_billing_list', $data);
  }
  ...
  public function billing($pid = "")
  {
    ...
    if (isset($post) && !empty($post)) {
      ...
      if ($this->form_validation->run() == TRUE) {
        ...
        $this->session->set_userdata('opd_booking_id', $booking_id);
        $this->session->set_flashdata('success', 'OPD Particular successfully saved.');
        redirect(base_url('emergency_booking/?status=print&type=1'));

      } else {
        $data['form_error'] = validation_errors();
      }
    }
    $this->load->view('emergency_booking/billing', $data);
  }
  ...
  public function print_consolidate_dbooking_report($id = "", $branch_id = '')
  {
    ...
    $payment_list = $this->emergency_booking->payment_list($booking_id, $branch_id);
    ...
    if (!empty($payment_list) && count($payment_list) > 1) {
      ...
      $data['total_payment'] = $this->emergency_booking->total_payment($booking_id, $branch_id, 2);
  ...
  public function print_blank_booking_report($id = "", $type = '')
  {
    ...
    $get_by_id_data = $this->emergency_booking->get_all_detail_print($booking_id);
    $template_format = $this->emergency_booking->template_format(array('section_id' => 1, 'types' => 1));
    $data['template_data'] = $template_format;
    ...
    $this->load->view('emergency_booking/print_blank_template_opd', $data);
  }
  ...
  public function advance_search()
  {
    ...
    $this->load->view('emergency_booking/advance_search', $data);
  }

  public function get_branch_data($branch_id)
  {
    if ($branch_id > 0) {
      $branchdata = $this->emergency_booking->branch_data($branch_id);
  ...
  {
    if ($branch_id > 0) {
      $simulationdata = $this->emergency_booking->get_simulation_data($branch_id);
      echo $simulationdata;
    }
  }
  ...
  {
    if ($branch_id > 0) {
      $doctor_data = $this->emergency_booking->get_referral_doctor_data($branch_id);
      echo $doctor_data;
    }
  }
  ...
  {
    if ($branch_id > 0) {
      $specialization_data = $this->emergency_booking->get_specialization_data($branch_id);
  ...
  {
    if ($branch_id > 0) {
      $attended_data = $this->emergency_booking->get_attended_doctor_data($branch_id);
      echo $attended_data;
    }
  }
  ...
  {
    $list = $this->emergency_booking->search_opd_data();
  ...
  public function eme_reg_excel()
  {
    ...
    header("Content-Disposition: attachment; filename=eme_reg_charge_list_" . time() . ".xls");
    header("Pragma: no-cache");
    header("Expires: 0");
    if (!empty($data)) {
      ob_end_clean();
  ...
  public function eme_reg_pdf()
  {
    $data['print_status'] = "";
    $data['data_list'] = $this->emergency_booking->search_opd_data();
    $this->load->view('emergency_booking/eme_reg_html', $data);
  ...
  public function opd_print()
  {
    $data['print_status'] = "1";
    $data['data_list'] = $this->emergency_booking->search_opd_data();
    $this->load->view('emergency_booking/opd_html', $data);
  }
  ...
  function get_test_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_test_vals($vals);
      if (!empty($result)) {
  ...
  function get_medicine_auto_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_medicine_auto_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  function get_dosage_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_dosage_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  function get_type_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_type_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  function get_duration_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_duration_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  function get_frequency_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_frequency_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  function get_advice_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_advice_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  {
    if ($branch_id > 0) {
      $diseases_data = $this->emergency_booking->get_diseases_data($branch_id);
      echo $diseases_data;
    }
  }
  ...
  {
    if ($branch_id > 0) {
      $source_from_data = $this->emergency_booking->get_source_from_data($branch_id);
  ...
  public function checkbox_list_save()
  {
    ...
    $branch_id = $users_data['parent_id'];
    $this->emergency_booking->delete_existing_branch_list_cols($module_id, $branch_id);
    $this->emergency_booking->insert_new_cols_branch_list_cols($branch_id, $module_id, $col_ids_array);
  ...
  {
    $update_status = $this->emergency_booking->update_doctor_status($opd_id, $status_type);
    echo $update_status;
    exit;
  }
  ...
  public function get_validity_date_in_between()
  {
    ...
    $result = $this->emergency_booking->get_validity_date_in_between($doctor_id, $booking_date, $patient_id);
    echo $result;
    die;
  }
  public function get_validate_date()
  {
    ...
    $booking_date = $this->input->post('booking_date');
    $patient_id = $this->input->post('patient_id');
    $result = $this->emergency_booking->get_validate_date($doctor_id, $booking_date, $patient_id);
    echo json_encode(array('date' => $result));
    exit;
  }
  ...
  public function import_opd_excel()
  {
    ...
    if (isset($_FILES) && !empty($_FILES)) {
      ...
      if ($this->form_validation->run() == TRUE) {
        ...
        $config['upload_path'] = DIR_UPLOAD_PATH . 'emergency_booking/';
        ...
        if (!$this->upload->do_upload('opd_list')) {
          ...
          echo "<pre> dfd";
          print_r(DIR_UPLOAD_PATH . 'emergency_booking/');
          exit;
        } else {
        ...
        if (!empty($arr_data)) {
          ...
          // echo 'dsd';die;
          $this->emergency_booking->save_all_opd_data($opd_all_data);
        }
        if (!empty($path)) {
          unlink($path);
        }
    ...
    $this->load->view('emergency_booking/import_opd_excel', $data);
  }
  ...
  function mlc_print($id = "", $branch_id = '')
  {
    ...
    $get_by_id_data = $this->emergency_booking->get_all_detail_print($opd_booking_id, $branch_id);
    // echo "<pre>"; print_r($get_by_id_data);die;
    $data['get_by_id_data'] = $get_by_id_data;
    $data['booking_id'] = $opd_booking_id;
    $this->load->view('emergency_booking/mlc_form_opd', $data);
  }
  ...
  public function partograph_pres($booking_id = "")
  {
    ...
    if ($booking_id > 0) {
      $this->load->model('patient/patient_model');
      $opd_booking_data = $this->emergency_booking->get_by_id($booking_id);
  ...
  {
    $result = $this->emergency_booking->dilated_stop($_POST['booked_id']);
  ...
  {
    $result = $this->emergency_booking->dilate_m_stop($_POST['booked_id']);
  ...
  {
    $patient_data = $this->emergency_booking->get_by_id($id);
  ...
  {
    $result = $this->emergency_booking->cyclo_start($_POST['booked_id']);
  ...
  {
    $result = $this->emergency_booking->cyclo_stop($_POST['booked_id']);
  ...
  {
    $result = $this->emergency_booking->cyclo_m_stop($_POST['booked_id']);
    $this->session->set_flashdata('success', 'Cyclo stoped.');
    echo 'Cyclo stoped';
  }
  ...
  public function update_patient_status($id = '')
  {
    ...
    $get_data = $this->emergency_booking->get_by_id_patient_details($id);
    $post = $this->input->post();
    $data['pat_data'] = $get_data;
    $data['form_error'] = [];
    if (isset($post) && !empty($post)) {
      $this->emergency_booking->change_arrive_status();
      echo 1;
      return false;
    }
    $this->load->view('emergency_booking/patient_status', $data);
  }
  ...
  public function upload_opd_prescription($booking_id = '')
  {
    ...
    if (isset($post) && !empty($post)) {
      ...
      if ($this->form_validation->run() == TRUE) {
        $config['upload_path'] = DIR_UPLOAD_PATH . 'emergency_booking/prescription/';
        ...
        if ($this->upload->do_upload('prescription_files')) {
          ...
          $this->emergency_booking->save_file($file_data['file_name']);
          echo 1;
          return false;
        } else {
    ...
    $this->load->view('emergency_booking/add_prescription_file', $data);
  }
  ...
  public function view_opd_files($booking_id)
  {
    $get_by_id_data = $this->emergency_booking->get_by_id($booking_id);
    $patient_name = $get_by_id_data['patient_name'];
    $this->load->model('emergency_booking/prescription_file_model', 'prescription_file');
    $data['page_title'] = "Prescription Files - For-" . $patient_name;
    $data['booking_id'] = $booking_id;
    $this->load->view('emergency_booking/view_prescription_files', $data);
  }
  ...
  public function ajax_file_list($booking_id = '')
  {
    ...
    foreach ($list as $prescription) {
      ...
      $sign_img = "";
      if (!empty($prescription->prescription_files) && file_exists(DIR_UPLOAD_PATH . 'emergency_booking/prescription/' . $prescription->prescription_files)) {
        $sign_img = ROOT_UPLOADS_PATH . 'emergency_booking/prescription/' . $prescription->prescription_files;
  ...
  {
    $result = $this->emergency_booking->updateidss();
  }
  ...
  function get_admission_dosage_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_admission_dosage_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  function get_admission_type_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_admission_type_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  function get_admission_duration_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_admission_duration_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  function get_admission_frequency_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_admission_frequency_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  function get_admission_advice_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_admission_advice_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  function get_nursing_dosage_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_nursing_dosage_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  function get_nursing_type_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_nursing_type_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  function get_nursing_duration_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_nursing_duration_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  function get_nursing_frequency_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_nursing_frequency_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  function get_nursing_advice_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_nursing_advice_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  {
    if ($examination_id > 0) {
      $examination_name = $this->emergency_booking->admission_examination_list($examination_id);
      echo $examination_name;
    }
  }
  ...
  {
    if ($diagnosis_id > 0) {
      $diagnosis_name = $this->emergency_booking->admission_diagnosis_name($diagnosis_id);
      echo $diagnosis_name;
    }
  }
  //hms_opd_suggetion
  ...
  {
    if ($suggetion_id > 0) {
      $suggetion_name = $this->emergency_booking->admission_suggetion_name($suggetion_id);
      echo $suggetion_name;
    }
  }
  ...
  {
    if ($personal_his_id > 0) {
      $personal_his_name = $this->emergency_booking->admission_personal_history_name($personal_his_id);
      echo $personal_his_name;
    }
  }
  ...
  {
    if ($pre_id > 0) {
      $pre_name = $this->emergency_booking->admission_prv_history_name($pre_id);
      echo $pre_name;
    }
  }
  ...
  {
    if ($complaints_id > 0) {
      $complaintsname = $this->emergency_booking->nursing_compalaints_list($complaints_id);
      echo $complaintsname;
    }
  }
  ...
  {
    if ($examination_id > 0) {
      $examination_name = $this->emergency_booking->nursing_examination_list($examination_id);
      echo $examination_name;
    }
  }
  ...
  {
    if ($diagnosis_id > 0) {
      $diagnosis_name = $this->emergency_booking->nursing_diagnosis_name($diagnosis_id);
      echo $diagnosis_name;
    }
  }
  //hms_opd_suggetion
  ...
  {
    if ($suggetion_id > 0) {
      $suggetion_name = $this->emergency_booking->nursing_suggetion_name($suggetion_id);
      echo $suggetion_name;
    }
  }
  ...
  {
    if ($personal_his_id > 0) {
      $personal_his_name = $this->emergency_booking->nursing_personal_history_name($personal_his_id);
      echo $personal_his_name;
    }
  }
  ...
  {
    if ($pre_id > 0) {
      $pre_name = $this->emergency_booking->nursing_prv_history_name($pre_id);
      echo $pre_name;
    }
  }
  ...
  function get_nursing_medicine_auto_vals($vals = "")
  {
    ...
    if (!empty($vals)) {
      $result = $this->emergency_booking->get_nursing_medicine_auto_vals($vals);
      if (!empty($result)) {
        echo json_encode($result, true);
      }
    }
  ...
  {
    if ($template_id > 0) {
      $templatedata = $this->emergency_booking->get_admission_template_data($template_id);
      echo $templatedata;
    }
  }
  ...
  {
    if ($template_id > 0) {
      $templatetestdata = $this->emergency_booking->get_admission_template_test_data($template_id);
      echo $templatetestdata;
    }
  }
  ...
  {
    if ($template_id > 0) {
      $templatetestdata = $this->emergency_booking->get_admission_template_prescription_data($template_id);
  ...
  {
    if ($template_id > 0) {
      $templatedata = $this->emergency_booking->get_nursing_template_data($template_id);
      echo $templatedata;
    }
  }
  ...
  {
    if ($template_id > 0) {
      $templatetestdata = $this->emergency_booking->get_nursing_template_test_data($template_id);
      echo $templatetestdata;
    }
  }
  ...
  {
    if ($template_id > 0) {
      $templatetestdata = $this->emergency_booking->get_nursing_template_prescription_data($template_id);
  ...
  {
    if ($pre_id > 0) {
      $pre_name = $this->emergency_booking->admission_illness_name($pre_id);
      echo $pre_name;
    }
  }
  ...
  {
    if ($pre_id > 0) {
      $pre_name = $this->emergency_booking->admission_obstetrics_menstrual_history_name($pre_id);
      echo $pre_name;
    }
  }
  ...
  {
    if ($pre_id > 0) {
      $pre_name = $this->emergency_booking->admission_family_history_disease_name($pre_id);
      echo $pre_name;
    }
  }
  ...
  public function print_label($ipd_id, $total_no)
  {
    ...
    if (!empty($ipd_id)) {
      $get_by_id_data = $this->emergency_booking->get_by_id($ipd_id);
      //print_r($get_by_id_data); die;
      ...
      $this->load->view('emergency_booking/print_label_template', $data);
    }
  }
  ...
  public function print_template($id)
  {
    ...
    //$post = $this->input->post();  
    $data['id'] = $id;
    $this->load->view('emergency_booking/template', $data);
